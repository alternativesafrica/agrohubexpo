<?php
	...
	class WS_Form_Submit extends WS_Form_Core {
		...
		public function db_read_expanded(&$submit_object, $expand_user = true, $expand_date_added = true, $expand_date_updated = true, $expand_status = true, $expand_actions = true, $expand_section_repeatable = true, $bypass_user_capability_check = false) {
		...
		public function db_read_all($join = '', $where = '', $order_by = '', $limit = '', $offset = '', $get_meta = true, $get_expanded = true, $bypass_user_capability_check = false, $clear_hidden_fields = false) {
			...
			foreach($return_array as $key => $submit) {
				...
				if($get_meta) {
					...
					// Get meta data
					$submit->meta = self::db_get_submit_meta($submit, false, $bypass_user_capability_check);

					// Clear hidden fields
					if($clear_hidden_fields) {

						$submit = self::clear_hidden_meta_values($submit);
					}
				}
		...
		public function db_read_by_hash($get_meta = true, $get_expanded = true, $form_id_check = true, $bypass_user_capability_check = false) {
			...
			if($get_expanded) {
				...
				self::db_read_expanded($submit_object, true, true, true, true, true, true, $bypass_user_capability_check);
			}
		...
		// Get submit meta
		public function db_get_submit_meta($submit, $meta_array = false, $bypass_user_capability_check = false) {
				...
				if($field_id > 0) {
					...
					switch($field_type) {
						...
						case 'datetime' :
							...
							// Already expanded?
							if(isset($value['mysql']) && isset($value['presentation_full'])) { break; }
							...
							if(is_string($value)) {
								...
								if(!empty($value)) {
									...
									$value_presentation_full = WS_Form_Common::get_date_by_type($value, $field_object);
								}
							}

							$value = array('mysql' => $value_mysql, 'presentation_full' => $value_presentation_full);
		...
		public function db_export_csv(&$file, $submit_ids = array(), $status = '', $date_from = '', $date_to = '') {
			...
			foreach($submits as $key => $submit) {
				...
				// Get meta data
				$submit->meta = self::db_get_submit_meta($submit);
				...
				foreach($csv_header_fields as $key => $value) {
					...
					switch($key) {
						...
						case 'duration' :
							...
							$row_array[] = isset($submit->{$key}) ? $submit->{$key} : '';
							break;

						default :

							$row_array[] = isset($submit->meta[$key]) ? $submit->meta[$key] : '';
					}
				}
				...
				foreach($submit_fields as $id => $field) {
					...
					// Get type
					$type = isset($submit->meta[$field_name]) ? (isset($submit->meta[$field_name]['type']) ? $submit->meta[$field_name]['type'] : '') : '';

					// Get value
					$value = isset($submit->meta[$field_name]) ? (isset($submit->meta[$field_name]['value']) ? $submit->meta[$field_name]['value'] : '') : '';
				...
				foreach($csv_header_ecommerce as $csv_header_ecommerce_single) {
					...
					// Get value
					$value = isset($submit->meta[$key]) ? $submit->meta[$key] : '';
		...
		public function clear_hidden_meta_values($submit = false, $bypass_user_capability_check = true) {
			...
			if(!isset($submit->meta)) { return $submit; }
			if(!isset($submit->meta['wsf_meta_key_hidden'])) { return $submit; }

			// Get section repeatable data (Unserialize if necesary)
			$section_repeatable_serialized = false;
			if(isset($submit->section_repeatable)) {

				$section_repeatable_serialized = is_serialized($submit->section_repeatable);

				$section_repeatable_array = $section_repeatable_serialized ? unserialize($submit->section_repeatable) : $submit->section_repeatable;
			...
			$meta_key_hidden_array = $submit->meta['wsf_meta_key_hidden'];
			...
			foreach($meta_key_hidden_array as $meta_key_hidden) {
				...
				if(
					!isset($submit->meta[$meta_key_hidden]) ||
					!isset($submit->meta[$meta_key_hidden]['id'])
				) {
					continue;
				}

				// Get field ID
				$field_id = intval($submit->meta[$meta_key_hidden]['id']);

				// Get section ID (Only set on repeatable sections)
				$section_id = isset($submit->meta[$meta_key_hidden]['section_id']) ? intval($submit->meta[$meta_key_hidden]['section_id']) : 0;
				if($section_id > 0) {
				...
				// Unset field
				unset($submit->meta[$meta_key_hidden]);

				// Unset fallback field
				unset($submit->meta[WS_FORM_FIELD_PREFIX . $field_id]);
			...
			if(count($field_ids_need_new_fallback) > 0) {
				...
				foreach($field_ids_need_new_fallback as $field_id) {
					...
					if(isset($meta_new[$field_name])) {
						...
						// Replace						
						$this->meta[$field_name] = $submit->meta[$field_name] = $meta_new[$field_name];
					}
				}
				...
				if($section_repeatable_edited) {
					...
					$this->section_repeatable = $submit->section_repeatable = ($section_repeatable_serialized ? serialize($section_repeatable_array) : $section_repeatable_array);
				}
			}
