	...
	class WS_Form_Action_Custom_Endpoint extends WS_Form_Action {
		...
		public function post($form, &$submit, $config) {
			...
			foreach($this->field_mapping as $field_map) {
				...
				$endpoint_key = WS_Form_Common::parse_variables_process($endpoint_key, $form, $submit, $content_type = 'text/plain');
			...
			foreach($this->custom_mapping as $custom_map) {
				...
				// Get value
				$endpoint_value = WS_Form_Common::parse_variables_process($custom_map['action_' . $this->id . '_endpoint_value'], $form, $submit, $content_type = 'text/plain');
			...
			foreach($this->header_mapping as $header_map) {
				...
				// Get header name
				$header_name = WS_Form_Common::parse_variables_process($header_map['action_' . $this->id . '_header_name'], $form, $submit, $content_type = 'text/plain');
				if(empty($header_name)) { continue; }

				// Get header value
				$header_value = WS_Form_Common::parse_variables_process($header_map['action_' . $this->id . '_header_value'], $form, $submit, $content_type = 'text/plain');
