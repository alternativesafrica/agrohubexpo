		...
		public static function parse_variables_process($parse_string, $form = false, $submit = false, $content_type = 'text/html', $scope = false, $depth = 1) {
			...
			if($scope === false) {
				...
				if(strpos($parse_string, 'submit') !== false) {
					...
					$variables['submit_id'] = self::get_object_var($submit, 'id', '');
					$variables['submit_user_id'] = self::get_object_var($submit, 'user_id', '');
					$variables['submit_hash'] = self::get_object_var($submit, 'hash', '');

					$form_id = self::get_object_var($form, 'id', '');

					if($form_id && $variables['submit_id']) {

						$variables['submit_admin_url'] = get_admin_url(null, 'admin.php?page=ws-form-submit&id=' . $form_id . '#' . $variables['submit_id']);
					...
					foreach($section->fields as $field) {
						...
						if($render_static_fields && $field_static) {
							...
							if($field_static === true) {
								...
								$mask_field = isset($field_type['mask_field_email']) ? $field_type['mask_field_email'] : '';
								if($mask_field == '') {
...
<script>
	...
	function wsf_review_nag_dismiss() {
		...
		(function($) {
			...
			$.ajax({ method: 'POST', url: '<?php echo esc_html(self::get_api_path('helper/review_nag/dismiss/')); ?>', data: { '_wpnonce': '<?php echo esc_attr(wp_create_nonce('wp_rest')); ?>' } });
...
<?php
		...
		public static function get_date_by_type($date, $field) {

			if(empty($date)) { return ''; }
			if(!is_string($date)) { return ''; }

			$format_date = self::get_object_meta_value($field, 'format_date', get_option('date_format'));
			if(empty($format_date)) { $format_date = get_option('date_format'); }
			$format_time = self::get_object_meta_value($field, 'format_time', get_option('time_format'));
			...
			switch($format_date) {
				...
				case 'd/m/Y' :
					...
					// Convert / to - so that strtime works with d/m/Y format
		...
		public static function get_encryption_status_html() {
			...
			if($ws_form_encryption->key_found === false) {			
				...
				$encryption_status_html .= '<p><strong style="color: #ff0000;">Caution:</strong> Once installed, do not change this key. Changing your encryption key at a later date will make it impossible to decrypt existing submission data. <a href="https://wsform.com/knowledgebase/data-encryption/" target="_blank">Learn More</a></p>';
