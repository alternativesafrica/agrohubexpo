<?php
	...
	class WS_Form_API_Field extends WS_Form_API {
		...
		public function api_cascade($parameters) {
			...
			$select_cascade_field_filter = intval(WS_Form_Common::get_object_meta_value($field, sprintf('%s_cascade_field_filter', $field_type), 0));
			...
			foreach($columns as $column_index => $column) {
				...
				if($column->id === $select_cascade_field_filter) { $data_column_index = $column_index; break; }
			}
