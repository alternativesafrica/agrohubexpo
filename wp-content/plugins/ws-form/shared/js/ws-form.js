(function($) {
	...
	$.WS_Form.prototype.get_section_html = function(section, section_repeatable_index) {
		...
		if(!this.is_admin) {
			...
			if(class_single_vertical_align) {
				...
				var class_single_vertical_align_config = this.get_field_value_fallback(false, false, 'class_single_vertical_align', false);
	...
	$.WS_Form.prototype.parse_variables_process = function(parse_string, section_repeatable_index, calc, field_to, calc_type, calc_register, depth) {
		...
		// Check calc_type
		if(typeof(calc_type) === 'undefined') { var calc_type = false; }

		// Check calc_register
		if(typeof(calc_register) === 'undefined') { var calc_register = true; }
...
//		}
	...
	$.WS_Form.prototype.language = function(id, value, html_encode, bypass_error) {
		...
		if(typeof bypass_error === 'undefined') { var bypass_error = false; }
	...
	$.WS_Form.prototype.get_field_html_single = function (field, value, is_submit, section_repeatable_index) {
		...
		if(is_submit) {
			...
			var mask_field_submit = this.get_field_value_fallback(field.type, label_position, 'mask_field_submit', false);
		...
		if(!is_submit) {
			...
			if(
				...
				var class_field_input_group = this.get_field_value_fallback(field.type, label_position, 'class_field_input_group', '');
		...
		switch(field.type) {
			...
			case 'file' :
				...
				// Get file_button_label default value
				var meta_key = $.WS_Form.meta_keys['file_button_label'];
				if(this.is_admin) {
				...
				var file_button_label = this.get_object_meta_value(field, 'file_button_label', meta_key_default);
				mask_values_field.file_button_label = file_button_label;
				...
				if(
					...
					if(file_name_array.length > 0){
						...
						mask_values_field.label = file_name_array.join(', ');
					}
				}
		...
		if(
			...
			var col_small_prepend_factor = this.get_field_value_fallback(field.type, label_position, 'col_small_prepend_factor', false);
			var col_small_append_factor = this.get_field_value_fallback(field.type, label_position, 'col_small_append_factor', false);
			if(
			...
			var mask_field_input_group_field = this.get_field_value_fallback(field.type, label_position, 'mask_field_input_group_field', '#field');
			...
			if(prepend !== '') {
				...
				var mask_field_input_group_prepend = this.get_field_value_fallback(field.type, label_position, 'mask_field_input_group_prepend', '#prepend');
			...
			if(append !== '') {
				...
				var mask_field_input_group_append = this.get_field_value_fallback(field.type, label_position, 'mask_field_input_group_append', '#append');
			...
			// Wrapper
			var mask_field_input_group = this.get_field_value_fallback(field.type, label_position, 'mask_field_input_group', '#field');
	...
	$.WS_Form.prototype.get_section_repeat_max = function(section, section_repeat_min) {
		...
		if(typeof(section_repeat_min) === 'undefined') { var section_repeat_min = this.get_section_repeat_min(section); }
	...
	$.WS_Form.prototype.get_section_repeat_default = function(section, section_repeat_min, section_repeat_max) {
		...
		if(typeof(section_repeat_min) === 'undefined') { var section_repeat_min = this.get_section_repeat_min(section); }
		if(typeof(section_repeat_max) === 'undefined') { var section_repeat_max = this.get_section_repeat_max(section, section_repeat_min); }
