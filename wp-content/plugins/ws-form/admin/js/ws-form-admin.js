(function($) {
	...
	$.WS_Form.prototype.form_render = function() {
		...
		this.form_obj.on('dragenter', function (e) {
			...
			$('.wsf-form-upload-json-window', $(this)).show();
		});
	...
	$.WS_Form.prototype.form_upload_ajax = function(form_data, status_bar, obj, success_callback, error_callback) {
		...
		var url = ws_form_settings.url + 'form/' + this.form_id + '/upload/json';
	...
	$.WS_Form.prototype.group_put_sort_index = function(obj) {
		...
		// Call AJAX request
		var call_obj = obj;
		this.api_call('group/' + group_id + '/sort_index/', 'POST', params, function(response) {
	...
	$.WS_Form.prototype.section_put_sort_index = function(obj) {
		...
		// Call AJAX request
		var call_obj = obj;
		$.WS_Form.this.api_call('section/' + section_id + '/sort_index/', 'POST', params, function(response) {
	...
	$.WS_Form.prototype.field_put_sort_index = function(obj) {
		...
		// Call AJAX request
		var call_obj = obj;
		$.WS_Form.this.api_call('field/' + field_id + '/sort_index/', 'POST', params, function(response) {
...
//		this.sidebar_reset();
	...
	$.WS_Form.prototype.sidebar_html = function(object, object_id, object_data, object_meta, repeater_meta_key, render_wrappers, render_field_wrappers, render_label, buttons, depth, inits, datetime_type) {
		...
		if(typeof(render_field_wrappers) === 'undefined') { var render_field_wrappers = true; }
				...
				for(var key in fieldset.meta_keys) {
					...
					if(
						...
						if(
							...
							meta_key_options.push('<li><a href="https://wsform.com/knowledgebase/calculated-fields/" title="' + this.language('calc') + '" target="_blank">' + this.svg('calc') + '</a></li>');
						}
					}
					...
					// Select2
					if(typeof(meta_key_config.select2) !== 'undefined') {
	...
	$.WS_Form.prototype.sidebar_image = function(obj_sidebar_inner) {
		...
		$('input[data-meta-key-type="image"]', obj_sidebar_inner).each(function() {
			...
			sidebar_image_button.click(function() {
	...
	$.WS_Form.prototype.sidebar_media = function(obj_sidebar_inner) {
		...
		$('input[data-meta-key-type="media"]', obj_sidebar_inner).each(function() {
			...
			sidebar_media_button.click(function() {
	...
	$.WS_Form.prototype.sidebar_repeater_row_html = function(repeater, repeater_data_row, repeater_meta_key) {
		...
		// Sort
		repeater_row_html += '<td data-icon><div data-action="wsf-repeater-row-sort" title="' + this.language('repeater_row_sort') + '">' + this.svg('sort') + '</div></td>'
	...
	$.WS_Form.prototype.sidebar_select_ajax = function(obj) {
		...
		$('select[data-select-ajax-method-search]', obj).each(function() {
			...
			if(data_select_ajax_id) { data_select_ajax_ids.push(data_select_ajax_id); }
		});
		...
		for(var data_meta_key in data_meta_keys) {
			...
			if(
				(typeof(ws_this.select_ajax_cache[data_meta_key]) === 'undefined') &&
				select_ajax_method_cache
			) {
	...
	$.WS_Form.prototype.sidebar_select_ajax_process = function(obj, data_meta_key) {
		...
		// Init select2 AJAX
		$('select[data-select-ajax-method-search][data-meta-key="' + data_meta_key + '"]', obj).each(function() {
			...
			if(typeof(select_ajax_id) !== 'undefined') {

				var option_text = (typeof(ws_this.select_ajax_cache[data_meta_key][select_ajax_id]) !== 'undefined') ? ws_this.select_ajax_cache[data_meta_key][select_ajax_id] : false;
				if(option_text !== false) {

					config.data = [ { id: select_ajax_id, text: option_text } ];
				}
			}

			// Add selection to select AJAX cache
			if((typeof($(this).select2()) === 'object') && select_ajax_method_search) {
	...
	$.WS_Form.prototype.data_grid_html = function(meta_key, meta_value, data_source_id, read_only) {
		...
		if((typeof(meta_key_config.compatibility_url) !== 'undefined') && $.WS_Form.settings_plugin.helper_compatibility) {
			...
			li_array.push('<li><div class="wsf-data-grid-compatibility"><a class="wsf-compatibility" href="' + meta_key_config.compatibility_url + '" target="_blank" title="' + this.language('field_compatibility') + '" tabindex="-1">' + this.svg('markup-circle') + '</a></div></li>');
		}
		...
		if(upload_download) {
			...
			if(!read_only) {
				...
				li_array.push('<li><div data-action="wsf-data-grid-upload" title="' + this.language('data_grid_group_upload_csv') + '">' + this.svg('upload') + '</div><input type="file" class="wsf-file-upload" id="wsf-data-grid-upload-file" accept=".csv"/></li>');
			}

			// Download CSV
			li_array.push('<li><div data-action="wsf-data-grid-download" title="' + this.language('data_grid_group_download_csv') + '">' + this.svg('download') + '</div></li>');
		}
	...
	$.WS_Form.prototype.data_grid_html_group = function(group, group_index, columns, rows_per_page, meta_key, read_only) {
		...
		if(!read_only) {
			...
			return_html += '<div data-action="wsf-data-grid-row-add" title="' + this.language('data_grid_row_add') + '"><div>' + this.svg('plus-circle') + '</div></div>';
		}
		...
		if(!read_only && group_settings_show) {
			...
			// Group - Settings - Open
			return_html += '<li class="wsf-flex-none"><div data-action="wsf-data-grid-settings" title="' + this.language('data_grid_settings') + '">' + this.svg('settings') + '</div></li>';
		}
	...
	$.WS_Form.prototype.data_grid_html_row = function(row, group_index, row_index, row_default, row_disabled, row_required, row_hidden, meta_key, read_only) {
		...
		for(var column_index = 0; column_index < column_count; column_index++) {
			...
			if(insert_image) {
				...
				return_html += '<div data-action="wsf-insert-image" title="' + this.language('data_grid_insert_image') + '">' + this.svg('file-picture') + '</div>';
			}
		...
		if(type_sub !== false) {
			...
			return_html += '<td data-fixed-icon><div data-action="wsf-data-grid-' + type_sub + '-clone" title="' + this.language('data_grid_' + type_sub + '_clone') + '">' + this.svg('clone') + '</div></td>';
		}
	...
	$.WS_Form.prototype.data_grid_init = function(obj, element) {
		...
		element.render = function(read_only) {
			...
			if(!read_only) {
				...
				// Auto group
				$('[data-action="wsf-data-grid-auto-group"]', obj).change(function() {
				...
				if(insert_image) {
					...
					$('[data-action="wsf-insert-image"]', obj).click(function() {
			...
			if(!read_only) {
				...
				// Action - Clone
				$('[data-action="wsf-data-grid-action-clone"]', obj).click(function(e) {

					// Save
					$.WS_Form.this.action_save();

					// Clone
					$.WS_Form.this.data_grid_row_clone($(this), element, meta_value);
				});
...
//						tolerance:			'pointer',
			...
			if(!read_only) {
				...
				$('[data-action="wsf-data-grid-column-add"]', obj).click(function() {
					...
					var column_width = $(".wsf-data-grid-table-inner thead th:not('[data-fixed-icon]'):last", obj).outerWidth();
	...
	$.WS_Form.prototype.data_source_render = function(obj, element) {
		...
		// On change
		$('[data-meta-key="data_source_id"]', obj).change(function() {
			...
			if(last_api_error !== '') {
				...
				$.WS_Form.this.api_call('field/' + object_id + '/last_api_error/clear/', 'POST', false);
			}
		});

		// On get data
		$('[data-meta-key="data_source_get"]').click(function() {
	...
	// Data grid - Clone row
	$.WS_Form.prototype.data_grid_row_clone = function(obj, element, meta_value) {
	...
	$.WS_Form.prototype.upload_status_bar = function(obj, render_file_name, render_file_size, render_abort) {
		...
		if(typeof(render_file_name) === 'undefined') { var render_file_name = true; }
		if(typeof(render_file_size) === 'undefined') { var render_file_size = true; }
		if(typeof(render_abort) === 'undefined') { var render_abort = true; }
	...
	$.WS_Form.prototype.sidebar_render = function(id) {
		...
		if(sidebar_nav) {
			...
			if(sidebar_url) {
				...
				var sidebar_button_html = '<li data-action-sidebar="' + id + '" title="' + this.html_encode(sidebar_label) + '" class="wsf-pro-required"><a href="' + this.get_plugin_website_url(sidebar_url, 'nav') + '" target="_blank" title="' + this.html_encode(sidebar_label) + '">' + this.svg(sidebar_icon) + '</a></li>';

			} else {

				var sidebar_button_html = '<li data-action-sidebar="' + id + '" title="' + this.html_encode(sidebar_label) + '">' + this.svg(sidebar_icon) + '</li>';
			}
		...
		obj_outer.html(this.sidebar_title(this.svg(sidebar_icon), sidebar_label, '', sidebar_kb_html, '', sidebar_expand));
	...
	$.WS_Form.prototype.history_pull = function(index, push_to_api, preview_update) {
		...
		if(typeof(preview_update) === 'undefined') { var preview_update = false; }
	...
	$.WS_Form.prototype.offset_change_init = function(obj) {
		...
		obj.find('.wsf-offset').last().mousedown(function() {
			...
			// Get number of columns
			var framework_column_count = parseInt(framework['columns']['column_count'], 10);
	...
	$.WS_Form.prototype.groups_sortable = function() {
		...
		obj_group_tabs.sortable({
			...
			cancel:				'.wsf-ui-cancel, input[type=text]:not([readonly]), li:not([data-id])',
			items:				'> li:not(.wsf-ui-cancel)',
	...
	$.WS_Form.prototype.wizards_populate = function(action_id) {
		...
		$.WS_Form.this.api_call('wizard/action/' + action_id, 'GET', [], function(response) {
			...
			for(var wizard_index in wizards) {
				...
				wizard_content += '<li><div class="wsf-template" data-action-id="' + $.WS_Form.this.html_encode(action_id) + '" data-list-id="' + $.WS_Form.this.html_encode(wizard_id) + '" title="' + $.WS_Form.this.html_encode(wizard_label) + '">';
				wizard_content += wizard_svg;
				wizard_content += '<div class="wsf-template-actions">';
	...
	$.WS_Form.prototype.wp_list_table_form = function() {
		...
		// Form actions
		$('[data-action]', form_list_table_obj).click(function() {
		...
		$('.wsf-form-upload-json-window', form_table_obj).on('drop', function (e) {
			...
			$.WS_Form.this.form_upload_json(files, $(this), function() {
				...
				$('.wsf-form-upload-json-window', form_table_obj).hide();
			});
		});
		...
		// Toggle status
		$('[data-action-ajax="wsf-form-status"]', form_table_obj).click(function() {
		...
		$('[data-action="wsf-clone"], [data-action="wsf-delete"], [data-action="wsf-export"]', form_table_obj).click(function(e) {

			e.preventDefault();
		});

		// Get form locations
		$('[data-action-ajax="wsf-form-locate"]', form_table_obj).click(function(e) {
		...
		$('[data-action-copy]', form_table_obj).click(function(e) {
	...
	$.WS_Form.prototype.wp_list_table_submit = function(form_id) {
		...
		// Change form
		$('#wsf_filter_id', submissions_obj).change(function() {
		...
		// Toggle status
		$('[data-action-ajax="wsf-submit-starred"]', submissions_obj).click(function(e) {
		...
		// Toggle viewed
		$('[data-action-ajax="wsf-submit-viewed"]', submissions_obj).click(function(e) {
		...
		// Prevent default on server side functions (Stops page jumping)
		$('[data-action="wsf-delete"], [data-action="wsf-export"]', submissions_obj).click(function(e) {
	...
	window.sidebar_action_open = function(obj, obj_form, obj_button) {
		...
		// Inner
		obj_outer.append('<div class="wsf-sidebar-inner"><fieldset class="wsf-fieldset"><div class="wsf-data-grid" data-object="form" data-id="' + obj.form_id + '" data-meta-key="action"></div></fieldset></div>');
		...
		$('[data-action="wsf-sidebar-save"]', obj_outer).click(function() {
			...
			obj.sidebar_action_save(false);
		});

		// Button - Save
		$('[data-action="wsf-sidebar-save-close"]', obj_outer).click(function() {

			obj.sidebar_action_save(true);
		});
		...
		$.WS_Form.this.keydown[83] = {'function': function() { obj.sidebar_action_save(true); }, 'ctrl_key': true};
